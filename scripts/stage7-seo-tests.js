...
jobs.push(test("category hub clean URLs are explicit redirects+rewrites without global cleanUrls", () => {
  ...
  const bookFallback = (vercel.rewrites || []).find(r => r.source === "/book" && r.destination === "/api/book-public");
  ...
  assert.strictEqual(bookRewrite.destination, "/api/book-public?id=:id");
  ...
  assert.ok(bookFallback, "missing /book rewrite to the public book handler");
  const htmlFallback = (vercel.rewrites || []).find(r => r.source === "/book.html" && r.destination === "/api/book-public");
  assert.ok(htmlFallback, "missing unresolved /book.html rewrite to the public book handler");
  ...
  const fallbackIdx = (vercel.rewrites || []).findIndex(r => r.source === "/book" && r.destination === "/api/book-public");
  const helperIdx = (vercel.rewrites || []).findIndex(r => r.source === "/book.html" && r.destination === "/api/legacy-book-redirect");
  const htmlFallbackIdx = (vercel.rewrites || []).findIndex(r => r.source === "/book.html" && r.destination === "/api/book-public");
  assert.ok(pathIdx >= 0 && pathIdx < fallbackIdx, "/book/:id rewrite must precede bare /book fallback");
  assert.ok(numericIdx >= 0 && numericIdx < pathIdx, "numeric book existence rewrite must precede non-numeric fallback");
  assert.ok(helperIdx >= 0 && helperIdx < htmlFallbackIdx, "numeric /book.html helper rewrite must precede the unresolved fallback");
  sitemap.PUBLIC_INFO_SLUGS.forEach(slug => {
    const redirect = (vercel.redirects || []).find(r => r.source === `/${slug}.html`);
    const rewrite = (vercel.rewrites || []).find(r => r.source === `/${slug}`);
    assert.ok(redirect, `missing redirect for ${slug}`);
