...
test("homepage sections keep ids and put books before category cards", () => {
  ...
  assert.match(html, /href="\/adabiyat"/);
  assert.match(html, /href="\/dini"/);
  assert.match(html, /href="\/children"/);
  assert.doesNotMatch(html, /href="adabiyat.html"/);
...
test("homepage category last card is centered on phone 2-column and icons are decorative", () => {
  ...
  assert.match(css, /@media \(min-width:701px\) and \(max-width:1100px\)\{[\s\S]*#bookCategories \.cards\{[\s\S]*grid-template-columns:repeat\(3,minmax\(0,1fr\)\) !important/);
  assert.match(css, /@media \(min-width:701px\) and \(max-width:1100px\)\{[\s\S]*#bookCategories \.cards \.card:last-child\{[\s\S]*grid-column:auto !important/);
});
