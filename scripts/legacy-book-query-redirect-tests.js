...
async function run() {
  ...
  await test("numeric legacy query rewrites to the helper before the detail shell", () => {
    ...
    const htmlShellIdx = rewriteIndex((rule) => rule.source === "/book.html" && rule.destination === "/api/book-public");
    ...
    const shellIdx = rewriteIndex((rule) => rule.source === "/book" && rule.destination === "/api/book-public");
    assert.ok(htmlIdx >= 0 && htmlIdx < htmlShellIdx);
    assert.ok(bareIdx >= 0 && pathIdx >= 0 && pathIdx < shellIdx);
    assert.ok(numericIdx >= 0 && numericIdx < pathIdx, "numeric /book/:id gate must precede the non-numeric fallback");
    assert.ok(bareIdx < shellIdx);
    const clean = (vercel.rewrites || []).find((rule) => rule.source === "/book/:id");
    assert.strictEqual(clean.destination, "/api/book-public?id=:id");
    assert.strictEqual(
