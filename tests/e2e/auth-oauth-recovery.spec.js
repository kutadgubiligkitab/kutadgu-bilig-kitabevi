...
test.describe("auth oauth vs recovery", () => {
  ...
  test("account.html loads current supabase-config and member.js pins", async ({ page }) => {
    ...
    expect(scripts.filter((src) => /supabase-config\.js/.test(src))).toEqual(["supabase-config.js?v=22"]);
    expect(scripts.filter((src) => /member\.js/.test(src))).toEqual(["member.js?v=28"]);
    expect(scripts.filter((src) => /account\.js/.test(src))).toEqual(["account.js?v=7"]);
