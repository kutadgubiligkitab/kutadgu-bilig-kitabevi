...
-- 2) كىتابلار
-- Current Production architecture: books.id is bigint GENERATED ALWAYS AS IDENTITY.
-- This CREATE is for fresh installs only. This file must NOT ALTER existing books.id.
...
-- book_id is bigint to match Production books.id (int8 identity). Never text.
do $$
declare
  v_books_udt text;
  v_hist_udt text;
begin
  select c.udt_name into v_books_udt
  from information_schema.columns c
  where c.table_schema = 'public' and c.table_name = 'books' and c.column_name = 'id';
  if v_books_udt is distinct from 'int8' then
    raise exception
      'Stage 65 setup aborted: public.books.id must be bigint/int8 (found %). This file will not alter books.id.',
      coalesce(v_books_udt, 'missing');
  end if;
  select c.udt_name into v_hist_udt
  from information_schema.columns c
  where c.table_schema = 'public' and c.table_name = 'book_price_history' and c.column_name = 'book_id';
  if v_hist_udt is not null and v_hist_udt is distinct from 'int8' then
    raise exception
      'Stage 65 setup aborted: public.book_price_history.book_id is %; expected bigint. Refusing destructive conversion.',
      v_hist_udt;
  end if;
end
$$;
...
drop function if exists public.rollback_book_price(text, bigint, numeric);
drop function if exists public.rollback_book_price(bigint, bigint, numeric);
create or replace function public.rollback_book_price(
  p_book_id bigint,
  p_history_id bigint,
  p_expected_price numeric
)
returns table(id bigint, price numeric, original_price numeric)
language plpgsql
security definer
set search_path = public
as $$
...
$$;
revoke all on function public.rollback_book_price(bigint, bigint, numeric) from public;
revoke execute on function public.rollback_book_price(bigint, bigint, numeric) from anon;
grant execute on function public.rollback_book_price(bigint, bigint, numeric) to authenticated;
