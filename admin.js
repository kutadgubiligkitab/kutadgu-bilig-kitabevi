...
let originalResetInFlight=false;
let originalResetDraft=null;
let priceHistoryInFlight=false;
let priceRollbackInFlight=false;
let priceHistoryDraft=null;
let priceHistoryRows=[];
let priceHistoryOffset=0;
let priceHistoryHasMore=false;
let priceRollbackDraft=null;
...
function stockLib(){
  return window.KutadguStock||Stock||{};
}
function isMissingStockColumnError(error){
  const msg=String(error&&error.message||"");
  const code=String(error&&error.code||"");
  const mentionsStock=/\bstock\b/i.test(msg);
  return mentionsStock&&(code==="42703"||code==="PGRST204"||/does not exist/i.test(msg)||/schema cache/i.test(msg));
}
function disableStockColumn(){
  ...
  spec.optionalColumns.stock=false;
  spec.optionalColumns.stock_status=false;
  window.KUTADGU_BOOKS_SCHEMA=spec;
  applyBooksSchema();
  renderUnconfiguredStock(null);
}
function enableStockColumn(){
  const spec=window.KUTADGU_BOOKS_SCHEMA||{optionalColumns:{}};
  spec.optionalColumns=spec.optionalColumns||{};
...
function renderUnconfiguredStock(count){
  const el=$("#adminUnconfiguredStock");
  if(!el)return;
  if(!presentBookCols.has("stock")||count==null){
    el.hidden=true;
    el.textContent="";
    return;
  }
  const lib=stockLib();
  el.hidden=false;
  el.textContent=lib.unconfiguredStockLabel?lib.unconfiguredStockLabel(count):`ئامبار سانى تەڭشەلمىگەن: ${count}`;
}
...
function isIdleBusy(){
  return !!(saveInFlight||importRunning||quickSaveInFlight||bulkInFlight||bulkPriceInFlight||bulkResetInFlight||originalCorrectInFlight||originalResetInFlight||priceHistoryInFlight||priceRollbackInFlight);
}
  ...
  try{
    ...
    if(presentBookCols.has("stock")){
      ...
      if(unconf.error&&isMissingStockColumnError(unconf.error)){
        ...
        renderUnconfiguredStock(unconf.count??0);
      }
    }else{
      renderUnconfiguredStock(null);
    }
  ...
  host.innerHTML=books.map(b=>`
    <article class="admin-book-row ${b.is_active===false?"admin-hidden-book":""}" data-book-id="${esc(b.id)}">
      ...
      <div>
        ...
        <div class="admin-book-meta">${esc(b.author||"—")} · ${esc(b.category||"")} · ${money(b.price)}${adminStockMetaHtml(b)}</div>
...
async function fingerprintRemoteCover(imageUrl,excludeId){
  const url=String(imageUrl||"").trim();
  if(!url)return null;
  if(isSampleDemoCoverUrl(url))throw new Error("ئۆرنەك ياكى سىناق مۇقاۋىسىنى ھەقىقىي كىتاب مۇقاۋىسى قىلىپ تەستىقلىغىلى بولمايدۇ.");
  const response=await fetch(url);
  if(!response.ok)throw new Error("مۇقاۋا يۈكلەنمىدى. تەستىق توختىتىلدى.");
  const blob=await response.blob();
  const file=new File([blob],"cover",{type:blob.type||"application/octet-stream"});
...
function pendingEditPayload(row,imageUrl,galleryUrls){
  ...
  if(imageUrl&&!isSampleDemoCoverUrl(imageUrl))payload.image_url=imageUrl;
...
function renderOriginalPriceStatus(value,opts){
  ...
  const resetBtn=$("#bookOriginalPriceResetBtn");
  ...
  const existing=!!(editing&&!isCreate);
  if(btn){
    btn.hidden=!presentBookCols.has("original_price")||!existing;
    btn.textContent=status.initialized?"ئەسلى باھانى تۈزىتىش":"ئەسلى باھانى بەلگىلەش";
  }
  if(resetBtn){
    const show=existing&&presentBookCols.has("original_price")&&Orig.canShowSingleBookReset&&Orig.canShowSingleBookReset({
...
function setSingleOriginalResetError(msg){
  const el=$("#singleOriginalResetError");
  if(!el)return;
  if(!msg){el.hidden=true;el.textContent="";return}
  el.hidden=false;
  el.textContent=msg;
}
function closeSingleOriginalResetModal(){
  if(originalResetInFlight)return;
  const modalEl=$("#singleOriginalResetModal");
  if(modalEl)modalEl.hidden=true;
  originalResetDraft=null;
  setSingleOriginalResetError("");
  const st=$("#singleOriginalResetStatus");
  if(st)st.textContent="";
  const save=$("#singleOriginalResetSaveBtn");
  if(save){save.disabled=false;save.textContent="ئەسلى باھاغا قايتۇرۇش"}
}
function openSingleOriginalResetModal(){
  if(saveInFlight||originalCorrectInFlight||originalResetInFlight)return;
  if(!presentBookCols.has("original_price"))return;
  const id=canonicalBookId(editing&&editing.id)||canonicalBookId($("#bookId")&&$("#bookId").value);
  if(!id){
    alert("كىتاب ID تېپىلمىدى. يېڭى قۇر يېزىلمايدۇ.");
  ...
  const planned=Orig.planSingleBookReset?Orig.planSingleBookReset({
    id,
    price:editing&&editing.price,
    original_price:editing&&editing.original_price
  }):{ok:false,error:"قايتۇرۇش يوق"};
  if(!planned.ok||!planned.write){
    setOriginalPriceNote(planned.error||"قايتۇرۇلمايدۇ.");
    return;
  }
  originalResetDraft={
    id,
    ...
    title:editing&&editing.title||($("#bookTitle")&&$("#bookTitle").value)||"",
    patch:planned.patch,
    newPrice:planned.price
  };
  const bookEl=$("#singleOriginalResetBook");
  if(bookEl)bookEl.textContent=originalResetDraft.title||"";
  const previewEl=$("#singleOriginalResetPreview");
  if(previewEl){
    const oldTxt=Orig.formatMoney?Orig.formatMoney(originalResetDraft.loadedPrice):(String(originalResetDraft.loadedPrice)+" ₺");
    const newTxt=Orig.formatMoney?Orig.formatMoney(originalResetDraft.newPrice):(String(originalResetDraft.newPrice)+" ₺");
    previewEl.textContent=`${oldTxt} → ${newTxt}`;
  }
  setSingleOriginalResetError("");
  const st=$("#singleOriginalResetStatus");
  if(st)st.textContent="";
...
async function persistSingleBookReset(id,patch,expected){
  if(Orig.assertPriceOnlyPatch)Orig.assertPriceOnlyPatch(patch);
  const canonical=canonicalBookId(id);
  if(!canonical)return {error:new Error("كىتاب ID تېپىلمىدى. يېڭى قۇر يېزىلمايدۇ.")};
  let result={error:null,data:[]};
  if(typeof window.__kutadguAdminResetOneBook==="function"){
    result=await window.__kutadguAdminResetOneBook(canonical,patch,expected);
  }else if(!db){
    return {error:new Error("Database يوق.")};
  }else{
    let query=db.from("books").update(patch).eq("id",canonical);
    query=Orig.applySingleResetCasFilter?Orig.applySingleResetCasFilter(query,expected):query;
    const {data,error}=await query.select("id,price,original_price");
    result={error,data};
  }
  if(result&&result.error)return {error:result.error};
  ...
  let existing=null;
  try{existing=await fetchBook(canonical)}catch(err){return {error:err}}
  const classified=Orig.singleResetCasResult?Orig.singleResetCasResult(rows,existing):{
    ok:false,
    error:existing?(Orig.STALE_RESET_ERROR||"باھا باشقا بەتتە ئۆزگەرتىلگەن. كىتابنى قايتا ئېچىپ قايتا سىناڭ."):(Orig.MISSING_BOOK_ERROR||"كىتاب تېپىلمىدى. يېڭى قۇر يېزىلمايدۇ.")
  };
  if(classified.ok)return {error:null,data:classified.data};
  return {error:new Error(classified.error)};
}
async function saveSingleOriginalReset(){
  if(originalResetInFlight)return;
  const draft=originalResetDraft;
  if(!draft||!draft.patch)return;
  originalResetInFlight=true;
  const save=$("#singleOriginalResetSaveBtn");
  if(save){save.disabled=true;save.textContent="ساقلىنىۋاتىدۇ..."}
  const st=$("#singleOriginalResetStatus");
  if(st)st.textContent="ساقلىنىۋاتىدۇ...";
  ...
  try{
    const fresh=await fetchBook(draft.id);
    if(!fresh)throw new Error("كىتاب تېپىلمىدى. يېڭى قۇر يېزىلمايدۇ.");
    if(Orig.isStaleSingleReset&&Orig.isStaleSingleReset({
      price:draft.loadedPrice,
      original_price:draft.loadedOriginal
    },fresh)){
      throw new Error(Orig.STALE_RESET_ERROR||"باھا باشقا بەتتە ئۆزگەرتىلگەن. كىتابنى قايتا ئېچىپ قايتا سىناڭ.");
    }
    const {error,data}=await persistSingleBookReset(draft.id,draft.patch,{
      price:draft.loadedPrice,
      original_price:draft.loadedOriginal
    });
    if(error)throw error;
    const saved=Array.isArray(data)&&data[0]?data[0]:null;
    const nextPrice=saved&&Object.prototype.hasOwnProperty.call(saved,"price")?saved.price:draft.newPrice;
    const nextOriginal=saved&&Object.prototype.hasOwnProperty.call(saved,"original_price")?saved.original_price:draft.loadedOriginal;
    if(editing&&String(editing.id)===String(draft.id))editing={...editing,price:nextPrice,original_price:nextOriginal};
    if($("#bookPrice"))$("#bookPrice").value=nextPrice;
    if(Prod.mergeBookPatch){
      books=Prod.mergeBookPatch(books,draft.id,{price:nextPrice});
      if(previewBooksMaster.length)previewBooksMaster=Prod.mergeBookPatch(previewBooksMaster,draft.id,{price:nextPrice});
    }
    if(Array.isArray(window.__kutadguAdminPreviewBooks)){
      window.__kutadguAdminPreviewBooks=Prod.mergeBookPatch?Prod.mergeBookPatch(window.__kutadguAdminPreviewBooks,draft.id,{price:nextPrice}):window.__kutadguAdminPreviewBooks;
    }
    invalidateBulkPricePreview();
    invalidateBulkResetPreview();
    if($("#bulkPriceModal")&&!$("#bulkPriceModal").hidden)renderBulkPricePreview();
    if($("#bulkResetModal")&&!$("#bulkResetModal").hidden)renderBulkResetPreview();
    renderOriginalPriceStatus(nextOriginal,{create:false});
    originalResetInFlight=false;
    closeSingleOriginalResetModal();
    setOriginalPriceNote("سېتىش باھاسى ئەسلى باھاغا قايتۇرۇلدى. ئەسلى باھا ئۆزگەرمىدى.");
    if(db)await Promise.all([loadBooks(),loadStats()]);
    else renderBooks();
  }catch(err){
    setSingleOriginalResetError(err.message||String(err));
    if(st)st.textContent="";
  }finally{
    originalResetInFlight=false;
    if(save){save.disabled=false;save.textContent="ئەسلى باھاغا قايتۇرۇش"}
...
function renderPriceHistoryButton(opts){
  const btn=$("#bookPriceHistoryBtn");
  if(!btn)return;
  const isCreate=!!(opts&&opts.create);
  const id=canonicalBookId(editing&&editing.id)||canonicalBookId($("#bookId")&&$("#bookId").value);
  btn.hidden=isCreate||!id;
}
function isMissingPriceHistory(error){
  const msg=String((error&&(error.message||error.code))||error||"");
  return /book_price_history|rollback_book_price|does not exist|42P01|42883|PGRST/i.test(msg)||(error&&(error.code==="42P01"||error.code==="42883"||error.code==="PGRST202"||error.code==="PGRST205"));
}
function setPriceHistoryError(msg){
  const el=$("#priceHistoryError");
  if(!el)return;
  if(!msg){el.hidden=true;el.textContent="";return}
  el.hidden=false;
  el.textContent=msg;
}
function setPriceRollbackError(msg){
  const el=$("#priceRollbackError");
  if(!el)return;
  if(!msg){el.hidden=true;el.textContent="";return}
  el.hidden=false;
  el.textContent=msg;
}
function resetPriceHistoryState(){
  priceHistoryDraft=null;
  priceHistoryRows=[];
  priceHistoryOffset=0;
  priceHistoryHasMore=false;
}
function closePriceHistoryModal(){
  if(priceRollbackInFlight)return;
  closePriceRollbackModal();
  const modalEl=$("#priceHistoryModal");
  if(modalEl)modalEl.hidden=true;
  resetPriceHistoryState();
  const more=$("#priceHistoryMoreBtn");
  ...
  const st=$("#priceHistoryStatus");
  if(st)st.textContent="";
}
function closePriceRollbackModal(){
  if(priceRollbackInFlight)return;
  const modalEl=$("#priceRollbackModal");
  if(modalEl)modalEl.hidden=true;
  priceRollbackDraft=null;
  setPriceRollbackError("");
  const st=$("#priceRollbackStatus");
  if(st)st.textContent="";
  const save=$("#priceRollbackSaveBtn");
  if(save){save.disabled=false;save.textContent="بۇ باھاغا قايتۇرۇش"}
}
function currentEditingPrice(){
  if(editing&&Object.prototype.hasOwnProperty.call(editing,"price"))return editing.price;
  const input=$("#bookPrice");
  if(input&&input.value!=="")return Number(input.value);
  return null;
}
function renderPriceHistoryMeta(){
  const title=priceHistoryDraft&&priceHistoryDraft.title||(editing&&editing.title)||"";
  const bookEl=$("#priceHistoryBook");
...
function renderPriceHistoryList(){
  ...
  const more=$("#priceHistoryMoreBtn");
  if(!list)return;
  const current=currentEditingPrice();
  if(!priceHistoryRows.length){
    list.innerHTML="";
    if(empty){
      empty.hidden=false;
      empty.textContent=(Hist.EMPTY_TEXT||"تېخى باھا ئۆزگىرىش تارىخى يوق.");
    }
    if(more)more.hidden=true;
    return;
  }
  if(empty)empty.hidden=true;
  list.innerHTML=priceHistoryRows.map(row=>{
  ...
  list.querySelectorAll("[data-history-rollback]").forEach(btn=>{
    btn.onclick=()=>openPriceRollbackModal(btn.dataset.historyRollback);
  });
  if(more)more.hidden=!priceHistoryHasMore;
}
async function fetchPriceHistoryPage(bookId,offset){
  const limit=Hist.PAGE_SIZE||20;
  if(typeof window.__kutadguAdminFetchPriceHistory==="function"){
    const result=await window.__kutadguAdminFetchPriceHistory(bookId,offset,limit);
    if(result&&result.error)return {error:result.error,rows:[],hasMore:false};
    return {
      error:null,
  ...
  if(error)return {error,rows:[],hasMore:false};
  const rows=Array.isArray(data)?data:[];
  const hasMore=rows.length>limit;
  return {error:null,rows:hasMore?rows.slice(0,limit):rows,hasMore};
}
async function loadPriceHistory({reset=false}={}){
  const id=priceHistoryDraft&&priceHistoryDraft.id;
  if(!id)return;
  if(priceHistoryInFlight||priceRollbackInFlight)return;
  if(reset){
    priceHistoryRows=[];
    priceHistoryOffset=0;
    priceHistoryHasMore=false;
  }
  priceHistoryInFlight=true;
  const st=$("#priceHistoryStatus");
  if(st)st.textContent="يۈكلىنىۋاتىدۇ...";
  setPriceHistoryError("");
  try{
    const page=await fetchPriceHistoryPage(id,priceHistoryOffset);
    if(page.error){
      if(isMissingPriceHistory(page.error))throw new Error(Hist.MISSING_TABLE_ERROR||"باھا تارىخى جەدۋىلى تېخى Database دا يوق. STAGE65_PRICE_HISTORY.sql نى Supabase SQL Editor دا Run قىلىڭ.");
      throw page.error;
    }
    priceHistoryRows=priceHistoryRows.concat(page.rows||[]);
    priceHistoryOffset=priceHistoryRows.length;
    priceHistoryHasMore=!!page.hasMore;
    renderPriceHistoryMeta();
    renderPriceHistoryList();
    if(st)st.textContent="";
  }catch(err){
    setPriceHistoryError(err.message||String(err));
    if(st)st.textContent="";
    if(!priceHistoryRows.length){
      const empty=$("#priceHistoryEmpty");
      if(empty)empty.hidden=true;
    ...
    priceHistoryInFlight=false;
  }
}
function openPriceHistoryModal(){
  if(saveInFlight||priceRollbackInFlight)return;
  const id=canonicalBookId(editing&&editing.id)||canonicalBookId($("#bookId")&&$("#bookId").value);
  if(!id){
    alert("كىتاب ID تېپىلمىدى. يېڭى قۇر يېزىلمايدۇ.");
    return;
  ...
  const modalEl=$("#priceHistoryModal");
  if(modalEl)modalEl.hidden=false;
  loadPriceHistory({reset:true});
}
function openPriceRollbackModal(historyId){
  if(priceRollbackInFlight)return;
  const row=priceHistoryRows.find(item=>String(item.id)===String(historyId));
  const id=priceHistoryDraft&&priceHistoryDraft.id||canonicalBookId(editing&&editing.id);
  const planned=Hist.planRollback?Hist.planRollback({
    bookId:id,
    historyRow:row,
    currentPrice:currentEditingPrice()
  }):{ok:false,error:"قايتۇرۇش يوق"};
  if(!planned.ok||!planned.canRollback){
    if(planned&&planned.alreadyCurrent){
      renderPriceHistoryList();
      return;
    }
    setPriceHistoryError((planned&&planned.error)||"قايتۇرۇشقا بولمايدۇ.");
    return;
  }
  priceRollbackDraft={
    bookId:id,
    historyId:row.id,
    historyRow:row,
    loadedPrice:currentEditingPrice(),
    targetPrice:planned.targetPrice,
    title:priceHistoryDraft&&priceHistoryDraft.title||(editing&&editing.title)||""
  };
...
async function persistPriceRollback(bookId,historyId,expectedPrice){
  ...
  if(!db)return {error:new Error("Database يوق."),data:[]};
  const {data,error}=await db.rpc("rollback_book_price",args);
  return {error,data};
}
function invalidatePriceRelatedPreviews(){
  invalidateBulkPricePreview();
  invalidateBulkResetPreview();
...
async function savePriceRollback(){
  if(priceRollbackInFlight)return;
  const draft=priceRollbackDraft;
  if(!draft)return;
  priceRollbackInFlight=true;
  const save=$("#priceRollbackSaveBtn");
  if(save){save.disabled=true;save.textContent="ساقلىنىۋاتىدۇ..."}
  const st=$("#priceRollbackStatus");
  if(st)st.textContent="ساقلىنىۋاتىدۇ...";
  setPriceRollbackError("");
  try{
    const fresh=await fetchBook(draft.bookId);
    if(!fresh)throw new Error(Hist.MISSING_BOOK_ERROR||"كىتاب تېپىلمىدى.");
    if(Hist.isStaleCurrentPrice&&Hist.isStaleCurrentPrice(draft.loadedPrice,fresh.price)){
      throw new Error(Hist.STALE_PRICE_ERROR||"باھا باشقا بەتتە ئۆزگەرتىلگەن. كىتابنى قايتا ئېچىپ قايتا سىناڭ.");
    }
    const {error,data}=await persistPriceRollback(draft.bookId,draft.historyId,draft.loadedPrice);
    if(error){
      if(isMissingPriceHistory(error))throw new Error(Hist.MISSING_TABLE_ERROR||"باھا تارىخى جەدۋىلى تېخى Database دا يوق. STAGE65_PRICE_HISTORY.sql نى Supabase SQL Editor دا Run قىلىڭ.");
      throw error;
    }
    const saved=Array.isArray(data)&&data[0]?data[0]:null;
    const nextPrice=saved&&Object.prototype.hasOwnProperty.call(saved,"price")?saved.price:draft.targetPrice;
    const nextOriginal=saved&&Object.prototype.hasOwnProperty.call(saved,"original_price")
      ?saved.original_price
      :(editing&&String(editing.id)===String(draft.bookId)?editing.original_price:null);
    if(editing&&String(editing.id)===String(draft.bookId)){
      editing={...editing,price:nextPrice,original_price:nextOriginal};
    }
    if($("#bookPrice"))$("#bookPrice").value=nextPrice==null||nextPrice===""?"":nextPrice;
    if(Prod.mergeBookPatch){
      books=Prod.mergeBookPatch(books,draft.bookId,{price:nextPrice});
      if(previewBooksMaster.length)previewBooksMaster=Prod.mergeBookPatch(previewBooksMaster,draft.bookId,{price:nextPrice});
    }
    if(Array.isArray(window.__kutadguAdminPreviewBooks)){
      window.__kutadguAdminPreviewBooks=Prod.mergeBookPatch?Prod.mergeBookPatch(window.__kutadguAdminPreviewBooks,draft.bookId,{price:nextPrice}):window.__kutadguAdminPreviewBooks;
    }
    invalidatePriceRelatedPreviews();
    renderOriginalPriceStatus(nextOriginal,{create:false});
    priceRollbackInFlight=false;
    closePriceRollbackModal();
    renderPriceHistoryMeta();
    await loadPriceHistory({reset:true});
    if(db)await Promise.all([loadBooks(),loadStats()]);
    else renderBooks();
  }catch(err){
    setPriceRollbackError(err.message||String(err));
    if(st)st.textContent="";
  }finally{
    priceRollbackInFlight=false;
    if(save){save.disabled=false;save.textContent="بۇ باھاغا قايتۇرۇش"}
...
async function openQuickEdit(id,trigger){
  ...
  if($("#quickEditId"))$("#quickEditId").dataset.currentImageUrl=b.image_url||"";
...
async function persistQuickEdit(id,patch){
  ...
  let result=await persistBookRow(payload,"UPDATE",id);
...
async function saveQuickEdit(e){
  ...
  const built=Prod.buildQuickEditPatch?Prod.buildQuickEditPatch(collectQuickEditInput(),{presentBookCols,currentImageUrl:$("#quickEditId")&&$("#quickEditId").dataset.currentImageUrl||""}):{ok:false,error:"تېز تەھرىر يوق"};
...
function refreshPreviewBooks(){
  ...
  if(presentBookCols.has("stock")){
    const lib=stockLib();
    renderUnconfiguredStock(lib.countUnconfiguredActiveBooks?lib.countUnconfiguredActiveBooks(master):null);
  }else{
    renderUnconfiguredStock(null);
  }
}
...
async function detectOptionalStockColumn(){
  if(!db)return;
  try{
    const {error}=await db.from("books").select("stock").limit(1);
...
function isMissingInteriorPrintTypeColumnError(error){
  const msg=String(error&&error.message||"");
  const code=String(error&&error.code||"");
  return /interior_print_type/i.test(msg)&&(code==="42703"||code==="PGRST204"||/does not exist/i.test(msg)||/schema cache/i.test(msg));
}
function disableInteriorPrintTypeColumn(){
  LIVE_OPTIONAL_BOOK_COLS.interior_print_type=false;
  const spec=window.KUTADGU_BOOKS_SCHEMA||{optionalColumns:{}};
  spec.optionalColumns=spec.optionalColumns||{};
  spec.optionalColumns.interior_print_type=false;
  window.KUTADGU_BOOKS_SCHEMA=spec;
  applyBooksSchema();
}
function enableInteriorPrintTypeColumn(){
  LIVE_OPTIONAL_BOOK_COLS.interior_print_type=true;
  const spec=window.KUTADGU_BOOKS_SCHEMA||{optionalColumns:{}};
  spec.optionalColumns=spec.optionalColumns||{};
  spec.optionalColumns.interior_print_type=true;
  window.KUTADGU_BOOKS_SCHEMA=spec;
  applyBooksSchema();
}
async function detectOptionalInteriorPrintTypeColumn(){
  if(!db)return;
  try{
    const {error}=await db.from("books").select("interior_print_type").limit(1);
    if(error&&isMissingInteriorPrintTypeColumnError(error)){
      disableInteriorPrintTypeColumn();
      return;
    }
    if(!error)enableInteriorPrintTypeColumn();
  }catch(err){
    console.warn(err);
  }
}
...
function releaseCoverSelection(){
  coverFormEpoch++;
...
function bindCoverPicker(){
  ...
  input.addEventListener("change",()=>{
    ...
    $("#bookCoverPreview").src=coverPreviewObjectUrl;
    $("#bookCoverPreview").style.visibility="visible";
...
const KNOWN_SAMPLE_COVER_SHA256=new Set([
  "596d3ed8ffab73b6c5b9059cd97cba5e12e9222da66dd0efe5bf3eaaa4aae183",
  "2e144fd20d419c3360b53dca21ce97df2f105b7f637cc48644b043fddb4bc6b9"
]);
const KNOWN_SAMPLE_COVER_DHASH=new Set([
  "6305088ce639c9a0",
  "6305088ce638c9a0"
]);
...
function isKnownSampleCoverFingerprint(sha,visual){
...
async function saveBook(e){
  ...
  const coverEpochAtSave=coverFormEpoch;
  const coverFile=$("#bookCover")&&$("#bookCover").files&&$("#bookCover").files[0]||null;
  function coverSelectionCurrent(){
    if(coverFormEpoch!==coverEpochAtSave)return false;
    const live=$("#bookCover")&&$("#bookCover").files&&$("#bookCover").files[0]||null;
    return live===coverFile;
  }
...
function catalogForSelectionHint(){
  ...
  return master.length?master:books;
}
function renderSelectedScopeHint(kind){
  const isPrice=kind==="price";
  const settings=isPrice?readBulkPriceSettings():readBulkResetSettings();
  const wrap=$(isPrice?"#bulkPriceSelectedWrap":"#bulkResetSelectedWrap");
  const hint=$(isPrice?"#bulkPriceSelectedHint":"#bulkResetSelectedHint");
  const list=$(isPrice?"#bulkPriceSelectedList":"#bulkResetSelectedList");
  const selected=settings.scope==="selected";
  if(wrap)wrap.hidden=!selected;
  if(!selected){
    if(hint){hint.textContent="";hint.className="admin-help"}
    if(list){list.hidden=true;list.innerHTML=""}
    return;
  }
  const info=Price.describeSelectedScope?Price.describeSelectedScope(settings.selectedIds,catalogForSelectionHint()):{
    count:0,
    empty:!settings.selectedIds.length,
    text:settings.selectedIds.length?`تاللانغان كىتاب: ${settings.selectedIds.length}`:(Price.EMPTY_SELECTED_MESSAGE||"ئالدى بىلەن كىتاب تىزىملىكىدىن كىتاب تاللاڭ."),
    titles:[]
  };
  if(hint){
    hint.textContent=info.text||"";
    hint.className=info.empty?"admin-status warn":"admin-help";
  }
  if(list){
    const titles=info.titles||[];
    list.hidden=!titles.length;
    list.innerHTML=titles.map(title=>`<li>${esc(title)}</li>`).join("");
  }
}
...
function syncBulkPriceScopeUi(){
  ...
  renderSelectedScopeHint("price");
  const previewBtn=$("#bulkPricePreviewBtn");
  const canPreview=Price.canRunPricePreview?Price.canRunPricePreview(settings):!(settings.scope==="selected"&&!settings.selectedIds.length);
  if(previewBtn)previewBtn.disabled=!canPreview||bulkPriceInFlight;
  syncBulkPriceConfirm();
}
...
async function runBulkPricePreview(){
  ...
  setBulkPriceError("");
  const settings=readBulkPriceSettings();
  if(Price.canRunPricePreview&&!Price.canRunPricePreview(settings)){
    setBulkPriceError(settings.scope==="selected"&&!(settings.selectedIds||[]).length
      ?(Price.EMPTY_SELECTED_MESSAGE||"ئالدى بىلەن كىتاب تىزىملىكىدىن كىتاب تاللاڭ.")
      :"ئالدىن كۆرۈش ئۈچۈن تەڭشەكلەرنى تولۇقلاڭ.");
    syncBulkPriceScopeUi();
    return;
  }
  const st=$("#bulkPriceStatus");
...
function syncBulkResetScopeUi(){
  ...
  if(catWrap)catWrap.hidden=settings.scope!=="category";
  renderSelectedScopeHint("reset");
  const previewBtn=$("#bulkResetPreviewBtn");
  const canPreview=Orig.canRunResetPreview?Orig.canRunResetPreview(settings):!(settings.scope==="selected"&&!settings.selectedIds.length);
  if(previewBtn)previewBtn.disabled=!canPreview||bulkResetInFlight;
  syncBulkResetConfirm();
}
...
async function runBulkResetPreview(){
  ...
  setBulkResetError("");
  const settings=readBulkResetSettings();
  if(Orig.canRunResetPreview&&!Orig.canRunResetPreview(settings)){
    setBulkResetError(settings.scope==="selected"&&!(settings.selectedIds||[]).length
      ?(Price.EMPTY_SELECTED_MESSAGE||"ئالدى بىلەن كىتاب تىزىملىكىدىن كىتاب تاللاڭ.")
      :"ئالدىن كۆرۈش ئۈچۈن تەڭشەكلەرنى تولۇقلاڭ.");
    syncBulkResetScopeUi();
    return;
...
async function confirmCoverRepair(){
  ...
  try{
    const fp=await fingerprintSelectedCover(file,book.id);
    const hash=fp.hash;
    const visualHash=fp.visual;
    const url=await uploadCover(book.id,file);
    const payload=CoverRepair.coverOnlyPayload(url);
    if(hash&&visualHash&&presentBookCols.has("cover_sha256")&&presentBookCols.has("cover_dhash")){payload.cover_sha256=hash;payload.cover_dhash=visualHash;}
    let repairQuery=db.from("books").update(payload).eq("id",book.id);
    if(book.image_url==null||book.image_url==="")repairQuery=repairQuery.is("image_url",null);
    else repairQuery=repairQuery.eq("image_url",book.image_url);
    const {error,data}=await repairQuery.select(COVER_REPAIR_SELECT);
    if(!error&&(!Array.isArray(data)||!data.length))throw new Error("بۇ كىتابنىڭ مۇقاۋىسى ئارىدا ئۆزگەرگەن. قايتا تەكشۈرۈڭ.");
...
async function confirmImport(){
  ...
  try{
    ...
    if(coverJobs.length){
      ...
      const acceptedCoverJobs=[];
      const seenCoverSha=new Map();
      const seenCoverVisual=new Map();
      for(const job of coverJobs){
        try{
          const fp=await fingerprintSelectedCover(job.file,job.id);
          ...
          coverFailed++;
          failedRows.push({rows:[job&&job.title],message:"مۇقاۋا يۈكلەنمىدى (كىتاب ئۆچۈرۈلمىدى؛ رېمونت بۆلىكىدىن قايتا باغلاڭ): "+(err.message||err)});
        }
      }
      const coverResults=await ImportCovers.mapPool(acceptedCoverJobs,ImportCovers.COVER_UPLOAD_CONCURRENCY,async job=>{
        const hash=job.hash||"";
        const visualHash=job.visual||"";
        const url=await uploadCover(job.id,job.file);
        const patch={image_url:url};
        if(hash&&visualHash&&presentBookCols.has("cover_sha256")&&presentBookCols.has("cover_dhash")){patch.cover_sha256=hash;patch.cover_dhash=visualHash;}
      ...
      coverResults.forEach((res,idx)=>{
        const job=acceptedCoverJobs[idx];
        if(res&&res.ok){
          coverOk++;
...
function bindBookListUx(){
  ...
  $("#closePriceHistory")&&($("#closePriceHistory").onclick=closePriceHistoryModal);
  $("#priceHistoryCloseBtn")&&($("#priceHistoryCloseBtn").onclick=closePriceHistoryModal);
  $("#priceHistoryMoreBtn")&&($("#priceHistoryMoreBtn").onclick=()=>loadPriceHistory({reset:false}));
  $("#priceHistoryModal")&&$("#priceHistoryModal").addEventListener("click",e=>{if(priceRollbackInFlight)return;if(e.target===$("#priceHistoryModal"))closePriceHistoryModal()});
  $("#closePriceRollback")&&($("#closePriceRollback").onclick=closePriceRollbackModal);
  $("#priceRollbackCancelBtn")&&($("#priceRollbackCancelBtn").onclick=closePriceRollbackModal);
  $("#priceRollbackSaveBtn")&&($("#priceRollbackSaveBtn").onclick=savePriceRollback);
  $("#priceRollbackModal")&&$("#priceRollbackModal").addEventListener("click",e=>{if(priceRollbackInFlight)return;if(e.target===$("#priceRollbackModal"))closePriceRollbackModal()});
  ...
  $("#closeSingleOriginalReset")&&($("#closeSingleOriginalReset").onclick=closeSingleOriginalResetModal);
  $("#singleOriginalResetCancelBtn")&&($("#singleOriginalResetCancelBtn").onclick=closeSingleOriginalResetModal);
  $("#singleOriginalResetSaveBtn")&&($("#singleOriginalResetSaveBtn").onclick=saveSingleOriginalReset);
  $("#singleOriginalResetModal")&&$("#singleOriginalResetModal").addEventListener("click",e=>{if(originalResetInFlight)return;if(e.target===$("#singleOriginalResetModal"))closeSingleOriginalResetModal()});
  ...
  document.querySelectorAll('input[name="bulkPriceScope"]').forEach(radio=>{
    radio.addEventListener("change",()=>{invalidateBulkPricePreview();syncBulkPriceScopeUi()});
  });
  $("#bulkPriceCategory")&&$("#bulkPriceCategory").addEventListener("change",()=>{invalidateBulkPricePreview();syncBulkPriceScopeUi()});
  $("#bulkPriceOperation")&&$("#bulkPriceOperation").addEventListener("change",()=>{invalidateBulkPricePreview();syncBulkPriceScopeUi()});
  $("#bulkPriceAmount")&&$("#bulkPriceAmount").addEventListener("input",()=>{invalidateBulkPricePreview();syncBulkPriceScopeUi()});
  ...
  document.querySelectorAll('input[name="bulkResetScope"]').forEach(radio=>{
    radio.addEventListener("change",()=>{invalidateBulkResetPreview();syncBulkResetScopeUi()});
  });
  $("#bulkResetCategory")&&$("#bulkResetCategory").addEventListener("change",()=>{invalidateBulkResetPreview();syncBulkResetScopeUi()});
  document.addEventListener("keydown",e=>{
    if(e.key!=="Escape")return;
    if($("#priceRollbackModal")&&!$("#priceRollbackModal").hidden){e.preventDefault();closePriceRollbackModal();return}
    if($("#priceHistoryModal")&&!$("#priceHistoryModal").hidden){e.preventDefault();closePriceHistoryModal();return}
    if($("#singleOriginalResetModal")&&!$("#singleOriginalResetModal").hidden){e.preventDefault();closeSingleOriginalResetModal();return}
